    ...
    for schema_name, schema in schemas.items():
        if schema.get("type") == "object":
            ...
            new_class = create_model(
                schema_name, __base__=(DocumentObject, BaseModel), **field_definitions
            )
...
async def run_server_(
    openapi_spec: Dict[str, Any], host: str = "localhost", port: int = 8888
):
    try:
...
def run_server():
    try:
        ...
        logger.warning(
            f"No file path provided, defaulting to 'openapi.json'. Error: {e}"
        )
