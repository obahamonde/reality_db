...
class TestRPCServer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_table(self, mock_create_table):
        ...
        with self.client.websocket_connect("/test") as websocket:
            ...
            self.assertEqual(
                response["result"]["message"], "Table TestTable created successfully"
            )
            self.assertEqual(response["result"]["id"], "TestTable")
            # Ensure the mocked method was called with correct parameters
            mock_create_table.assert_awaited_once_with(
                prefix="test", table_name="TestTable"
            )
    ...
    async def test_get_item(self, mock_get_item):
        ...
        with self.client.websocket_connect("/test") as websocket:
            request = {
                "method": "GetItem",
                "properties": {"table_name": "TestTable", "id": "item1"},
                "id": str(uuid4()),
            }
            websocket.send_json(request)
            response = websocket.receive_json()
            ...
            mock_get_item.assert_awaited_once_with(
                prefix="test", table_name="TestTable", item_id="item1"
            )
    ...
    async def test_delete_item(self, mock_delete_item):
        ...
        with self.client.websocket_connect("/test") as websocket:
            ...
            self.assertEqual(response["status"], "success")
            self.assertEqual(
                response["result"]["message"], "Item 'item1' deleted successfully"
            )
            self.assertEqual(response["result"]["id"], "item1")
            mock_delete_item.assert_awaited_once_with(
                prefix="test", table_name="TestTable", item_id="item1"
            )
    ...
    async def test_batch_get_item(self, mock_batch_get_item):
        ...
        with self.client.websocket_connect("/test") as websocket:
            request = {
                "method": "BatchGetItem",
                "properties": {"table_name": "TestTable", "ids": ["item1", "item2"]},
                "id": str(uuid4()),
            }
            websocket.send_json(request)
            response = websocket.receive_json()
            ...
            mock_batch_get_item.assert_awaited_once_with(
                prefix="test", table_name="TestTable", ids=["item1", "item2"]
            )
    ...
    async def test_batch_write_item(self, mock_batch_write_item):
        ...
        with self.client.websocket_connect("/test") as websocket:
            ...
            request = {
                ...
                "properties": {"table_name": "TestTable", "items": items},
                "id": str(uuid4()),
            }
            websocket.send_json(request)
            response = websocket.receive_json()
            ...
            # Updated assertion
            self.assertEqual(
                [item.model_dump(exclude={"object"}) for item in kwargs["items"]], items
            )
    ...
    async def test_get_item_nonexistent(self, mock_get_item):
        ...
        with self.client.websocket_connect("/test") as websocket:
            request = {
                "method": "GetItem",
                "properties": {"table_name": "TestTable", "id": "nonexistent"},
                "id": str(uuid4()),
            }
            websocket.send_json(request)
            response = websocket.receive_json()
            ...
            mock_get_item.assert_awaited_once_with(
                prefix="test", table_name="TestTable", item_id="nonexistent"
            )
